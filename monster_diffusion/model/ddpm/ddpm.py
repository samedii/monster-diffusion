...
class DDPM(nn.Module):
    def __init__(self):
        ...
        ResnetBlock = functools.partial(
            ResnetBlockDDPM,
            act=act,
            temb_dim=4 * nf + config.model.conditioning_channels,
            dropout=dropout,
        )
        ...
        modules.append(conv3x3(in_ch, config.model.out_channels, init_scale=0.0))
        self.all_modules = nn.ModuleList(modules)

        # self.scale_by_sigma = config.model.scale_by_sigma

    def forward(self, x, ts, conditioning):
        modules = self.all_modules
        m_idx = 0

        # timestep/scale embedding
        temb = layers.get_timestep_embedding(ts, self.nf)
        temb = modules[m_idx](temb)
        m_idx += 1
        temb = modules[m_idx](self.act(temb))
        m_idx += 1

        temb = torch.cat([temb, conditioning], dim=1)

        # h = torch.cat(
        #     [x, F.interpolate(variational_features, size=x.shape[-2:])], dim=1
        # )
        h = x
